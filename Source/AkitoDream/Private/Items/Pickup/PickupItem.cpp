...
float APickupItem::TransformedSin()
{
	return Amplitude * FMath::Sin(RunningTime * TimeConstant);;
}

float APickupItem::TransformedCos()
{
...
void APickupItem::OnSphereBeginOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	if (UserInterface)
	{
		UserInterface->AddMeows(this);
		SpawnPickupSound();
		Destroy();
	}
}

void APickupItem::SpawnPickupSystem()
{
	if (PickupEffect)
	{
...
void APickupItem::SpawnPickupSound()
{
	if (PickupSound)
	{
